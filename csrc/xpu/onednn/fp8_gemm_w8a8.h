...
static inline void dnnl_matmul_w8a8_fp8(
  ...
  // block quant param
  bool is_block_quant = (m1_sc.dim() == 2) && (m2_sc.dim() == 2) && (m1_sc.size(1) != 1) && (m2_sc.size(1) != 1);
  int64_t group_size = -1;
  if (is_block_quant) {
    TORCH_CHECK(m1_sc.size(1) == m2_sc.size(1), "Mismatch group size in input and weight.", m1_sc.size(1), " vs ", m2_sc.size(1));
